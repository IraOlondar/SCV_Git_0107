...
git log
``` 
для вывода списка коммитов, или команндой:
```
git log --oneline
```
...
Для переходы на шужный комит в терминале вводим команду git checkout, где в качестве аргуиента указыавется хеш нужного коммита. Пример:
```
git checkout 2bbdbe3
```
при этом все наши файлы примут вид по состоянию на момент создания коммита с хешем 2bbdbe3.
Для переменения на последний коммит, необходимо в терминале выполнить команду:
```
git checkout master
```